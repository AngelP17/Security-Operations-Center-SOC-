...
DEMO_EVENTS = [
    ...
    },
    {
        ...
        "severity": "high",
        "ip": "192.168.1.201",
        "desc": "Failed SSH login attempt (root)",
    },
    {
        "type": "new_device",
        "severity": "medium",
        "ip": "192.168.1.201",
        "desc": "New device discovered on network",
    },
    {
...
def api_network_scan():
    ...
    try:
        ...
        """,
            (
                local_ip,
                f"Network scan completed: {len(discovered)} devices found on {subnet}.0/24",
            ),
        )
        conn.commit()
        conn.close()
...
def api_traffic_data():
    ...
    for row in rows:
        ...
        if hour in traffic_by_hour:
            ...
            traffic_by_hour[hour]["recv"] = (
                int(row["total_recv"] / (1024 * 1024)) if row["total_recv"] else 0
            )
            traffic_by_hour[hour]["sent"] = (
                int(row["total_sent"] / (1024 * 1024)) if row["total_sent"] else 0
            )
    ...
    return jsonify({"labels": labels, "inbound": inbound, "outbound": outbound})
...
def collect_traffic_data():
    ...
    while _traffic_collector_running:
        try:
            ...
            cur.execute(
                "DELETE FROM traffic_history WHERE timestamp < datetime('now', '-7 days')"
            )
            conn.commit()
            conn.close()
